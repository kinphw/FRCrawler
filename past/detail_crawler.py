...
from past.models import DetailItem, ListItem, CombinedItem
from past.config import PASTREQ_DETAIL_URL, DEFAULT_HEADERS, ST_NO, MU_NO, ACT_CD
from past.utils import html_to_text_preserve_p_br, get_td_html_after_th, random_sleep
...
class DetailCrawler:
    ...
    def get_detail_html(self, pastreq_idx: int) -> str:
        """
        상세 내용 HTML을 요청하여 반환
        
        Args:
            pastreq_idx: 상세화면 pastreqIdx 값
        ...
        # 요청 및 응답
        response = requests.post(PASTREQ_DETAIL_URL, headers=self.headers, data=data)
        return response.text
        
    def parse_detail_html(self, html_content: str, pastreq_idx: int) -> DetailItem:
        """
        상세 내용 HTML을 파싱하여 DetailItem 객체 반환
        
        Args:
            html_content: 상세 페이지 HTML
            pastreq_idx: 문서 식별자 (디버깅용)
        ...
        type_text = None
        inquiry = None
        factual_background = None  # 법령해석요청의 원인이 되는 사실관계 (추가)
        answer = None
        reason = None
        ...
        if not soup:
            # 실패 항목 기록
            self.failed_items.append((pastreq_idx, "HTML 파싱 실패"))
            return DetailItem(
                title="[파싱 오류]",
                serial_number=None,
                type_text=None,
                inquiry=None,
                factual_background=None,  # 추가
                answer=None,
                reason=None
            )
        ...
        # 각 필드 추출 (과거 회신사례의 필드명에 맞게 조정)
        serial_number = get_td_content_after_th("일련번호")
        type_text = get_td_content_after_th("타입")
        inquiry = get_td_content_after_th("질의요지")
        
        # 법령해석요청의 원인이 되는 사실관계 추가
        factual_background = get_td_content_after_th("법령해석요청의 원인이 되는 사실관계")
        ...
        # 법령해석요청의 원인이 되는 사실관계가 없으면 정규식으로 찾기 시도
        ...
        return DetailItem(
            ...
            type_text=type_text,
            inquiry=inquiry,
            factual_background=factual_background,  # 추가
            answer=answer,
            reason=reason
        )
        
    def get_detail_item(self, pastreq_idx: int) -> Optional[DetailItem]:
        """
        pastreq_idx 값으로 상세 내용을 가져와 파싱
        
        Args:
            pastreq_idx: 문서 식별자 (pastreqIdx)
        ...
        try:
            html_content = self.get_detail_html(pastreq_idx)
            detail_item = self.parse_detail_html(html_content, pastreq_idx)
            return detail_item
        except Exception as e:
            # 실패 항목 기록 (터미널 출력 없이)
            self.failed_items.append((pastreq_idx, str(e)))
            return None
    
    def process_item(self, list_item: ListItem) -> CombinedItem:
        """단일 항목 처리를 위한 helper 함수 (병렬 처리용)"""
        detail_item = self.get_detail_item(list_item.pastreqIdx)
    ...
    def process_list_items(self, list_items: List[ListItem]) -> List[CombinedItem]:
        ...
        if self.failed_items:
            ...
            # 처음 3개만 상세 출력
            for i, (idx, error) in enumerate(self.failed_items[:3]):
